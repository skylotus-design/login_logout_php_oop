...
<head>
    ...
    <?php include('../kontrol/registrasi_kontrol.php');?>
    <meta charset="UTF-8">
...
<body>
    ...
    <?php
        if($_SERVER["REQUEST_METHOD"] == "POST"){
            ...
            $pasCek = $registrasi->sandiCek($pass1, $pass2);
            if($pasCek){
                $emilCek = $registrasi->emailCek($email);
                if($emilCek){
                    echo "Alamat email sudah ada";
                }else {
                    $register = $registrasi->registrasi($nama, $email, $pass1);
                    if($register){
